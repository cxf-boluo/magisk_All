...
mv -f /data/misc/user/0/cacerts-added/* $MODDIR/system/etc/security/cacerts  >> $MODDIR/post-fs.log
...
if [ -n "$selinux_context" ] && [ "$selinux_context" != "?" ]; then
    chcon -R $selinux_context $MODDIR/system/etc/security/cacerts      >> $MODDIR/post-fs.log
else
    chcon -R $default_selinux_context $MODDIR/system/etc/security/cacerts     >> $MODDIR/post-fs.log
fi